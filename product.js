...
// ===== NEW FUNCTION: HANDLE SELLER LINK BY USERNAME =====
function applyFallbackUserData(usernameInput) {
console.log("🔍 Evaluating username for seller profile link generation:", usernameInput);

// Target elements that might represent the seller profile link/container
const profileLinks = document.querySelectorAll('.seller-profile-link, [data-seller-link], #shopNameHeader,.shop-name-area');

// ===== UPDATED CLEAN LOGIC =====
let cleanUsername = null;

if (usernameInput!== null && usernameInput!== undefined) {
    const value = String(usernameInput).trim(); // number হলেও string বানাবে

    if (value!== "") {
        cleanUsername = encodeURIComponent(
            value.replace(/^@/, "") // @ থাকলে কেটে দিবে
        );
    }
}
// ===== END =====
...
if (cleanUsername) {
    const generatedUrl = `https://www.ghotimarket.com/seller?@${cleanUsername}`;
    console.log("✅ Valid username detected. Generated Seller URL:", generatedUrl);
    console.log("ℹ️ Fallback was NOT triggered.");

    // If there are profile link elements in the DOM, update them with the valid username-only URL
    const links = document.querySelectorAll('a.seller-profile-link, [data-seller-profile-url]');
    links.forEach(link => {
    ...
    console.warn("⚠️ Fallback Triggered: Username is missing, null, undefined, empty, whitespace, or invalid.");
    console.log("🚫 Generated URL: NONE (Fallback active)");

    // Disable all profile links completely as per strict requirements
    ...
    links.forEach(link => {
        ...
        link.addEventListener('click', (e) => {
            e.preventDefault();
            e.stopPropagation();
            console.warn("🚫 Click blocked: Seller profile link is disabled because no valid username was found.");
        }, { capture: true });
    });

    const shopNameEl = document.getElementById("shopName");
    if (shopNameEl) {
        shopNameEl.style.pointerEvents = 'none';
    }
...
if (sellerId) {
    if (userCache.has(sellerId)) {
        ...
        // CHANGED: Now use username only
        applyFallbackUserData(cachedUser.username);
        ...
        getDoc(doc(db, "users", sellerId)).then(userSnap => {
            if (userSnap.exists()) {
                ...
                // CHANGED: Now use username only
                applyFallbackUserData(userData.username);
                ...
                applyFallbackUserData(null); // Disable link
            }
        }).catch(err => {
